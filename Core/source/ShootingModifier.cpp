    ...
    void ShootingModifier::onUpdate(float elapsedTime)
    {
        ...
        if (this->timer >= this->shotPattern.delays[this->currentSalvo])
        {
            ...
            for (unsigned int i = 0; i < this->shotPattern.shotSalvos[this->currentSalvo].shotMovementNames.size(); ++i)
            {
                ...
                modifiers.push_back(new ProjectileModifier(se::Vector2(0.0f, 0.0f), 50.0f));
                modifiers.push_back(new MovementPatternModifier(Spawner::getMovementPattern(this->shotPattern.shotSalvos[this->currentSalvo].shotMovementNames[i]), this->entity->getSprite().getRotation() + this->shotPattern.shotSalvos[this->currentSalvo].rotations[i], this->shotPattern.shotSalvos[this->currentSalvo].speeds[i], MovementPatternModifier::Style::Kill));
